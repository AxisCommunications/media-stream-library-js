...
describe('Sdp', () => {
  it('should expose correct controlURIs', () => {
    expect(Sdp.extractURIs(example))
      .toEqual([
  ...
  it('should parse to a JS object', () => {
    expect(Sdp.parse(example)).toEqual(
      { session: {
        version: '0',
        origin: { username: '-',
  ...
  it('messageFromBuffer should produce a message with the correct structure', () => {
    const message = Sdp.messageFromBuffer(example)
    expect(Object.keys(message))
      .toEqual(['type', 'data', 'sdp'])

    expect(message.data).toBe(example)
    expect(message.type).toEqual(SDP)
  })
})
