 ...
 * @return {undefined}
 */
const runComponentTests = (component, name = '') => {
  ...
  test(`${name} should have objectMode on all streams`, () => {
    ...
    incoming.ReadableState && expect(incoming.ReadableState.objectMode).toBe(true)
    incoming.WritableState && expect(incoming.WritableState.objectMode).toBe(true)
    outgoing.ReadableState && expect(outgoing.ReadableState.objectMode).toBe(true)
    outgoing.WritableState && expect(outgoing.WritableState.objectMode).toBe(true)
  })
}
