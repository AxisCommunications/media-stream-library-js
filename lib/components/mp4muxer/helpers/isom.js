...
const createParameterSetArrayClass = function (sizeMask = 0x00) {
  return class ParameterSetArray {
    /**
...
QTFF/QTFFChap3/qtff3.html#//apple_ref/doc/uid/TP40000939-CH205-124774'
  ...
  */
  ...
  tfdt: {
    ...
    body: [
      ['baseMediaDecodeTime', UInt64BE, 0]
    ]
  },
  trun: {
    container: 'traf',
...
class Box {
  ...
  /**
   * Write a value directly into a data representation of the box,
   * bypassing changing the box itself. This can be used to modify e.g.
   * a data template directly.
   * @param  {Object} config     A map of element names and values
   * @param  {Buffer} buffer     The target buffer with box data
   * @param  {Number} [offset=0] The number of bytes into the target to consider
   *                             as start of the box data.
   * @return {undefined}
   */
   ...
   */
  print (indent = 0) {
    console.warn(' '.repeat(indent), `[${this.type}] (${this.byteLength})`)
    for (const [key, entry] of this.struct) {
      const element = entry.element
      if (element.print) {
        element.print(indent + 2)
      } else {
        console.warn(' '.repeat(indent + 2), key, '=', element.value, `(${element.byteLength})`)
      }
    }
  }
}
