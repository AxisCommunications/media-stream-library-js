...
const aacEncodingName = (audioConfigBytes) => {
  ...
  const channels = CHANNEL_CONFIG_NAMES[channelConfig] || channelConfig
...
const aacSettings = (media, date, trackId) => {
  ...
  return {
    ...
    sampleEntryBox: new Container('mp4a', {
      'samplerate': (Number(media.rtpmap.clockrate) << 16) >>> 0 // FIXME: Is this  correct?
    }, new Box('esds', {
