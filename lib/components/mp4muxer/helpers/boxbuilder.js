...
class BoxBuilder {
  ...
  moov (sdp, date) {
    ...
    sdp.media.forEach((media) => {
      const payloadType = (media.rtpmap && media.rtpmap.payloadType) ? media.rtpmap.payloadType : 0
      const encoding = (media.rtpmap && media.rtpmap.encodingName) ? media.rtpmap.encodingName : undefined
      ...
      if (formatDefaults[encoding]) {
        ...
        trackData.clockrate = (media.rtpmap && media.rtpmap.clockrate) ? media.rtpmap.clockrate : 0
