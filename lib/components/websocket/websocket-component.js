...
const openWebSocket = require('./helpers/openwebsocket')
...
class WebSocketComponent extends Component {
  ...
  constructor (socket) {
    ...
    socket.onmessage = (msg) => {
      ...
      if (!incoming.push({data: buffer, type: RAW})) {
        ...
        if (socket.readyState === window.WebSocket.OPEN) {
          debug('live:websocket:incoming')('downstream frozen')
          socket.close()
        }
      }
    }
    ...
    outgoing.on('finish', () => {
      debug('live:websocket:outgoing')('finish')
      if (socket.readyState !== window.WebSocket.CLOSED) {
        socket.close()
      }
    })
     ...
     */
    socket.onclose = (e) => {
      debug('live:websocket:close')(e.code)
   ...
   */
  static open (config) {
    return openWebSocket(config).then((socket) => new WebSocketComponent(socket))
  }
}
