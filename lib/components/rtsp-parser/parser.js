const { Rtsp, Sdp } = require('../../utils/protocols')
const { RTP, RTCP, RTSP } = require('../messageTypes')
...
class Parser {
  ...
  _parseRtsp () {
    ...
    const bodyOffset = Rtsp.bodyOffset(lastChunk)
    ...
    const rtspHeaderLength = this._length - lastChunk.length + bodyOffset
    ...
    if (rtspHeaderLength === buffer.length || buffer[rtspHeaderLength] === ASCII_DOLLAR) {
      ...
      messages.push({ type: RTSP, data: packet })
      ...
      messages.push({ type: RTSP, data: packet })
      messages.push(Sdp.messageFromBuffer(body))
    }
  ...
  _parseInterleaved () {
    ...
    // Extract messages
    if (channel % 2 === 0) { // Even channels 0, 2, ...
      messages.push({ type: RTP, data: packet, channel })
    } else { // Odd channels 1, 3, ...
      let rtcpPackets = packet
      do { // RTCP packets can be packed together, unbundle them:
