...
const {frames, setupResponse, rtspRtpRtcpCombined, rtspWithTrailingRtp, sdpResponse} =
  require('./fixtures')
const {RTP, RTSP, RTCP, SDP} = require('../messageTypes')
...
describe('Parsing of interleaved data', () => {
  ...
  test('can append buffers', () => {
    expect(() => { parser.parse(Buffer.alloc(0)) }).not.toThrow()
  })
  ...
  describe('error handling', () => {
    test('should handle a [36, 0, x] buffer correctly', () => {
      ...
      expect(parser._length).toBe(3) // The buffer should remain until more data arrives
    })
  ...
  describe('1 buffer = 1 rtp package', () => {
    ...
    beforeAll(() => {
      ...
      frames.onePointZero.map((byte, index) => { buffer1[index] = byte })
    })
  ...
  describe('1 buffer = 1,5 rtp package', () => {
    ...
    beforeAll(() => {
      ...
      frames.onePointFive.map((byte, index) => { buffer15[index] = byte })
    })
  ...
  describe('2 buffers = 1,5 +0,5 rtp package', () => {
    let buffer15, buffer05
    beforeAll(() => {
      buffer15 = Buffer.alloc(frames.onePointFive.length)
      frames.onePointFive.map((byte, index) => { buffer15[index] = byte })
      buffer05 = Buffer.alloc(frames.zeroPointFive.length)
      frames.zeroPointFive.map((byte, index) => { buffer05[index] = byte })
    })
  ...
  describe('RTSP package', () => {
    ...
    beforeAll(() => {
      ...
      setupResponse.split('').map(
        (character, index) => { RtspBuffer[index] = character.charCodeAt(0) })
    })
    ...
    test('should find RTSP, RTP and RTCP packages in the same buffer', () => {
      ...
      expect(messages[0].type).toEqual(RTSP)
      expect(messages[1].type).toEqual(RTP)
      expect(messages[1].channel).toEqual(0)
      expect(messages[2].type).toEqual(RTCP)
      expect(messages[2].channel).toEqual(1)
      expect(messages[3].type).toEqual(RTCP)
  ...
  describe('SDP data', () => {
    ...
    test('should extract twice, once with the full RTSP and once with the SDP data', () => {
      ...
      expect(messages[0].type).toEqual(RTSP)
      expect(messages[1].type).toEqual(SDP)
