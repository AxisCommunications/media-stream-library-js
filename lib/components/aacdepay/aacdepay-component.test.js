const AACComponent = require('./aacdepay-component')
const {Sdp, Rtp} = require('../../utils/protocols')
const {RTP, ELEMENTARY} = require('../messageTypes')
const validateComponent = require('../../utils/validate-component')
/* eslint-disable */
const sdpMessage = Sdp.messageFromBuffer(
  Buffer.from(`
v=0
...
describe('is a valid component', () => {
  const c = new AACComponent()
  validateComponent(c, 'mp4muxer component')
})
...
test('Emits an AAC package with headers cut', () => {
  ...
  expect(msg.data.length + 4).toEqual(Rtp.payload(rtpMessage.data).length)
})
