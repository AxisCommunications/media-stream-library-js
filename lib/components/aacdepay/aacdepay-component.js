...
const {RTP, SDP} = require('../messageTypes')
...
class AACDepayComponent extends Component {
  constructor () {
    let AACPayloadType = 97 // default for Axis cameras
    let hasHeader
    ...
    const incoming = new Transform({
      ...
      transform (msg, encoding, callback) {
        if (msg.type === SDP) {
          ...
          if (validMedia) {
            AACPayloadType = Number(validMedia.rtpmap.payloadType) || AACPayloadType
            ...
            hasHeader =
              (sizeLength + Math.max(indexLength, indexDeltaLength) +
              CTSDeltaLength + DTSDeltaLength +
              RandomAccessIndication + StreamStateIndication +
              AuxiliaryDataSizeLength) > 0
          }
          callback(null, msg)
        } else if (msg.type === RTP && Rtp.payloadType(msg.data) === AACPayloadType) {
          aacDepay(msg, hasHeader, this.push.bind(this))
          callback()
        } else {
...
module.exports = AACDepayComponent
