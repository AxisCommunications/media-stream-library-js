...
export function makeQuantHeader(precision, qTable) {
...
export function makeFrameHeader(width, height, type) {
    return Buffer.from([
        0xff,
        0xc0,
        0,
...
export function makeHuffmanHeader() {
    ...
    return Buffer.concat(__spread(LUM_DC_BUFFER.map(Buffer.from), LUM_AC_BUFFER.map(Buffer.from), CHM_DC_BUFFER.map(Buffer.from), CHM_AC_BUFFER.map(Buffer.from)));
}
