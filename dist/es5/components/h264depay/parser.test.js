...
describe('h264 handler', function () {
    ...
    it('parses a single NALU packet', function () {
        ...
        var remaining = h264depay(Buffer.alloc(0), { type: MessageType.RTP, data: singleNalu, channel: 0 }, callback);
        expect(callback).toHaveBeenCalledTimes(1);
        expect(remaining.length).toEqual(0);
        var msg = callback.mock.calls[0][0];
    ...
    it('parses a FU-A frame split on two RTP packages', function () {
        ...
        /* eslint-enable */
        var remaining = h264depay(Buffer.alloc(0), { type: MessageType.RTP, data: fuaPart1, channel: 0 }, callback);
